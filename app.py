...
def forecast_lstm(on_date):
    # pull enough history for look_back + indicator warm-up
    start = on_date - timedelta(days=look_back*3)
    df = yf.download("7010.SR", start=start, end=on_date)
...
def forecast_arima():
    # arima_model was trained with pmdarima; predict 1 period ahead
    return float(arima_model.predict(n_periods=1)[0])

def forecast_es():
    # ExponentialSmoothing from statsmodels; forecast 1 step
...
st.markdown(
    "**Actual** close price from Yahoo Finance, plus **predicted** close for tomorrow "
...
if st.button("Forecast Next Day"):
    ...
    lstm_pred  = forecast_lstm(date.today())
    rnn_pred   = forecast_rnn(date.today())
    arima_pred = forecast_arima()
    es_pred    = forecast_es()

    # layout: four columns
    col1, col2, col3, col4 = st.columns(4)
    col1.metric("LSTM",            f"{lstm_pred:.2f} SAR")
    col2.metric("Simple RNN",      f"{rnn_pred:.2f} SAR")
    col3.metric("ARIMA",           f"{arima_pred:.2f} SAR")
    col4.metric("Exp. Smoothing",  f"{es_pred:.2f} SAR")
